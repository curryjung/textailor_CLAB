...
# from torch.nn import functional as conv2d_gradfix
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style, use_modulation=True):
        ...
        if not use_modulation:
            style = torch.ones_like(self.modulation(style).view(batch, 1, in_channel, 1, 1))
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, use_modulation=True):
        out = self.conv(input, style, use_modulation=use_modulation)
        # out = out + self.bias
...
class Generator(nn.Module):
    def __init__(
        ...
        self.conv1 = StyledConv(
            512, 512, 3, style_dim, blur_kernel=blur_kernel,demodulate=demodulate
        )
    ...
    def forward(
        ...
        # block1
        out = self.conv1(out, latent[:, 0], use_modulation=use_modulation)
        ...
        for conv1, conv2, to_rgb in zip(
            ...
            out = conv1(out, latent[:, i], use_modulation=use_modulation)
            out = conv2(out, latent[:, i + 1], use_modulation=use_modulation)
...
class Content_Encoder(nn.Module):
    ...
    def forward(self, input):
        out = self.convs(input) #out shape:
        [1,512,4,16]
        return out
...
if __name__ == "__main__":
    ...
    test_generator = Generator(demodulate=False)

    content_img = torch.randn(4,3,64,256)
    content_img = functional.resize(content_img, (256,256))

    content_latent = test_encoder(content_img)

    content_latent = torch.randn((4,512,16,16))


    styles = [torch.ones(4,14,512)]

    output = test_generator(content_latent, styles, input_is_latent=True, use_content_input=True, use_modulation=False)
