...
class clab_SwapSynthesizer(BaseSynthesizer):

    @staticmethod
    def add_commandline_args(parser):
        parser.add_argument("--save_each", type=str2bool, default=False)
        return parser
    ...
    def prepare_synthesis(self):
        ...
        print(f"content:{self.folders[0]}")
        print(f"style:{self.folders[1]}")
        self.contents = self.get_dataset(self.folders[0], force_square=False)
        self.styles = self.get_dataset(self.folders[1], force_square=False)
